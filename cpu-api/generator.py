...
class CodeGeneratorRunnable:
    ...
    def add_exit(self):
        ...
        if len(self.waiting_for[self.curr_thread]) > 0:
            ...
            print(
                "  thread %s has outstanding children:" % self.curr_thread,
                self.waiting_for[self.curr_thread],
            )
            exit(1)
        self.tab()
        self.fd.write('End("%s");\n' % self.curr_thread)
    ...
    def parse(self):
        ...
        while curr < len(tokens):
            if tokens[curr] == "":
                ...
                self.abort_if(
                    len(tokens) < curr + 2,
                    "malformed program: fork needs process name,sleep length plus {description} of what the forked process should do",
                )
                args = tokens[curr + 1]
                args_split = args.split(",")
                self.abort_if(
                    len(args_split) != 2,
                    "not enough args to fork (process name, sleep time)",
                )
                ...
                self.abort_if(
                    wait_count[fork_level] != 0, "#waits does not match #forks"
                )
                brace_count -= 1
                fork_level -= 1
...
parser.add_option(
    ...
    "--readable",
    default="read",
    help='file to read (e.g., "read", to produce read.c)',
    action="store",
    type="string",
    dest="readable",
)
parser.add_option(
    ...
    "--runnable",
    default="run",
    help='file to run (e.g., "run", to produce run.c)',
    action="store",
    type="string",
    dest="runnable",
)
...
parser.add_option(
    ...
    "--fork_chance",
    default=30,
    help="chances that a program will fork",
    action="store",
    type="int",
    dest="fork_chance",
)
parser.add_option(
    ...
    "--wait_chance",
    default=40,
    help="chances that a program will wait",
    action="store",
    type="int",
    dest="wait_chance",
)
parser.add_option(
    ...
    "--exit_chance",
    default=30,
    help="chances that a program will exit",
    action="store",
    type="int",
    dest="exit_chance",
)
parser.add_option(
    ...
    "--sleep_time",
    default=10,
    help="max sleep time for a process",
    action="store",
    type="int",
    dest="max_sleep_time",
)
parser.add_option(
    ...
    "--action_list",
    default="none",
    help='action list, instead of randomly generated ones (simple example: "fork b,10 {} wait" is a program that runs a process (called a) which then forks process b which runs for 10 seconds, and then a waits for b to complete; see README for details',
    action="store",
    type="string",
    dest="action_list",
)
